...
class Lopta(Widget):
    ...
    def ball_collide(self, player):
        if self.collide_widget(player):
            pocx = (self.x + player.x)/2  #koordinata u kojima se lopta
            pocy = (self.y + player.y)/2  # i igrač dodiruju, poc = point of collision
            if pocy == self.y:
                k = 0
            else:
...
class Igrac(Widget):
    ...
    def player_collide(self, player):
        if self.collide_widget(player):
            pocx = (self.x + player.x)/2  #koordinate dodira
            pocy = (self.y + player.y)/2  #dve kugle
            if pocy == self.y:
                k = 0
            else:
                k = (self.x - pocx)/(pocy - self.y) #derivacija u točki dodira kugla, kako bi mogao odrediti kut pod kojima će se one odbiti
...
class SoccerStars(App):
    def build(self):
        igra = Igra()
        Clock.schedule_interval(igra.update, 1.0 / 100.0) #funkcija update se poziva 60x u sekundi
        return igra
